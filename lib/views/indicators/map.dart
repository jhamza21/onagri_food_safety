...
class _MapPageState extends State<MapPage> {
  ...
  void initState() {
    ...
    _filterData(_data, widget.range);
  ...
  _filterData(List<Model> data, RangeValues range) {
    List<Model> _filtredData = [];
    for (int i = 0; i < data.length; i++) {
      if (int.parse(data[i].stateCode) >= range.start.toInt() &&
          int.parse(data[i].stateCode) <= range.end.toInt())
        _filtredData.add(data[i]);
    }
    _data = _filtredData;
  }
    ...
    return Padding(
      ...
      child: Container(
        child: Center(
          child: SfMaps(
            title:
                const MapTitle("Taux d'insécurité alimentaire dans la Tunisie"),
                  ...
                  return showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          ...
                          content: Text("Le taux d'insécurité alimentaire à " +
                              _data[selected].state +
                              " est : " +
